...
```
kubectl apply -f openresty-pod-hostnetwork.yaml
kubectl cp hello-r2ai.html openresty:/usr/local/openresty/nginx/html
...
curl your_ip/hello-r2ai # 这里会返回服务端地址（Pod地址）
...
### 部署Pod（Hello R2AI! ConfigMap）
```
kubectl apply -f openresty-pod-hostnetwork-configmap.yaml

# 查看下文件，是否存在
kubectl exec -it openresty -- cat /usr/local/openresty/nginx/html/hello-r2ai.html
