...
kubectl exec -it busybox -- wget -q -O - openresty/hello-r2ai.html
kubectl exec -it busybox -- wget -q -O - openresty/hello-r2ai # 这里会返回服务端地址（Pod地址），地址会变化
...
kubectl get service
curl your_ip:your_port/hello-r2ai.html
curl your_ip:your_port/hello-r2ai # 这里会返回服务端地址（Pod地址），地址会变化
...
# 获取service， 这里可以看到映射的端口 (PORT(S))
kubectl get service
curl your_ip:30080/hello-r2ai.html
curl your_ip:30080/hello-r2ai # 这里会返回服务端地址（Pod地址），地址会变化
...
curl your_ip/hello-r2ai.html
curl your_ip/hello-r2ai # 这里会返回服务端地址（Pod地址），地址会变化
